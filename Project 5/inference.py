...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            allPossible = util.Counter()
            jailpositions = self.getJailPosition()
            allPossible[jailpositions] = 1.0
        else:
            a = 0
            while a < len(self.legalPositions):
                distance = util.manhattanDistance(self.legalPositions[a], pacmanPosition)
                allPossible[self.legalPositions[a]] = emissionModel[distance] * self.beliefs[self.legalPositions[a]]
                a += 1
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while i < self.numParticles:
            j = 0
            while j < len(self.legalPositions):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            while i < len(self.legalPositions):
                allPos[self.legalPositions[i]] += emissionModel[
                                                      util.manhattanDistance(
                                                          self.legalPositions[i],
                                                          pacmanPosition)] * old[self.legalPositions[i]]
                i += 1
            if any(allPos.values()):
                temp = []
                k = 0
                while k < self.numParticles:
                    temp.append(util.sample(allPos))
                    k += 1
                self.particles = temp
            else:
                self.initializeUniformly(gameState)
    ...
    def elapseTime(self, gameState):
        ...
        while i < len(self.particles):
            if self.particles[i] in newPosDistance.keys():
                temp = newPosDistance[self.particles[i]]
            else:
                temp = self.getPositionDistribution(self.setGhostPosition(gameState, self.particles[i]))
                newPosDistance[self.particles[i]] = temp
            tempParticles.append(util.sample(temp))
            i += 1
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        permut = list(itertools.product(pos, repeat=self.numGhosts))
        random.shuffle(permut)
        i = 0
        while i < self.numParticles:
            self.particles.append(permut[i % len(permut)])
            i += 1
    ...
    def observeState(self, gameState):
        ...
        if len(noisyDistances) < self.numGhosts:
            return
        emissionModels = [busters.getObservationDistribution(distance) for distance in noisyDistances]
        # print emissionModels
        i = 0
        while i < len(self.particles):
            j = 0
            while j < len(self.particles[i]):
                ...
                j += 1
            i += 1
        pValue = set(self.particles)
        w = util.Counter()
        for v in pValue:
            w[v] = 1
            k = 0
            while k < self.numGhosts:
                if noisyDistances[k] is not None:
                    w[v] *= emissionModels[k][util.manhattanDistance(v[k], pacmanPosition)]
                else:
                    pass
                k += 1
            w[v] *= self.particles.count(v)
        if w.totalCount() == 0:
            self.initializeParticles()
            return
        w.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            temp = 0
            while temp < self.numGhosts:
                newParticle[temp] = util.sample(getPositionDistributionForGhost(setGhostPositions(
                                                            gameState,
                                                            newParticle),
                                                            temp, self.ghostAgents[temp]))
                temp += 1
            newParticles.append(tuple(newParticle))
        self.particles = newParticles

    def getBeliefDistribution(self):
        pValues = set(self.particles)
        b = util.Counter()
        for v in pValues:
            x = self.particles.count(v)
            y = float(self.numParticles)
            b[v] = x / y
        return b
