...
def enhancedFeatureExtractorDigit(datum):
    ...
    allPixels = datum.getPixels()
    features = basicFeatureExtractorDigit(datum)
    breaks = 0
    nonzero = 0
    firstLeft = None
    aboveCenter = 0
    for eachPixel in allPixels:
        active = False
        j = 1
        while j < len(eachPixel):
            if eachPixel[j] != 0:
                nonzero += 1
                if not firstLeft or j < firstLeft:
                    firstLeft = j
                if j <= (len(allPixels) + 1) / 2:
                    aboveCenter += 1
            if eachPixel[j] != eachPixel[j - 1]:
                breaks += 1
            j += 1

    j = 0
    width = len(allPixels[0]) - (firstLeft * 2)
    pastRight = 0
    firstTop = None
    while j < len(allPixels[0]):
        active = False
        col = [p[j] for p in allPixels]
        i = 1
        while i < len(col):
            if col[j] != 0:
                nonzero += 1
                if not firstTop or i < firstTop:
                    firstTop = i
                if i <= (len(allPixels[0]) + 1) / 2:
                    pastRight += 1
            if col[i] != eachPixel[i - 1]:
                breaks += 1
            i += 1
        j += 1

    height = len(allPixels) - (firstTop * 2)
...
def enhancedPacmanFeatures(state, action):
    ...
    """
    features = util.Counter()
    features["STOP"] = int(Directions.STOP == action) * 100
    successorState = state.generateSuccessor(0, action)
    ghostPositions = successorState.getGhostPositions()
    pacmanPosition = successorState.getPacmanPosition()

    allCapsules = successorState.getCapsules()
    allFood = state.getFood()
    f = [(x, y) for x, row in enumerate(allFood) for y, f in enumerate(row) if f]
    ghosts = sorted([util.manhattanDistance(pacmanPosition, i) for i in ghostPositions])
    features["nearest_ghost"] = ghosts[0] * 1.0

    i = 0
    while i < min(len(ghosts), 1):
        features[("ghost", i)] = 5 / (0.1 + ghosts[i])
        i += 1

    caps = sorted([util.manhattanDistance(pacmanPosition, i) for i in allCapsules])

    i = 0
    while i < min(len(caps), 1):
        features[("capsule", i)] = 15 / (1 + caps[i])
        i += 1

    nearestFood = sorted([util.manhattanDistance(pacmanPosition, i) for i in f])

    for i, w in zip(range(min(len(nearestFood), 5)), [1.3, 0.8] + [0.9] * 3):
        features[("food", i)] = w * nearestFood[i]

        features["capsule count"] = len(allCapsules) * 10
